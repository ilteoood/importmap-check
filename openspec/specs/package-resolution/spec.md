...
- Whether later versions should support custom npm dist-tags as update targets beyond the built-in set, allowing users to opt into channel-aware comparison (e.g. `--target preview` or `--target canary`)
